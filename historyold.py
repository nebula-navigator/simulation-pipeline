...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 30 14:31:17 2024
...
axs[0, 0].plot(df['time[Myr]'], df['massNew[Msun](10)'], label='Mass Evolution', color='w')
...
for event_type in ordered_event_types + additional_event_types:
    ...
    axs[0, 0].scatter(event_points['time[Myr]'], event_points['massNew[Msun](10)'],
...
axs[0, 0].scatter(comp_type_14_points['time[Myr]'], comp_type_14_points['massNew[Msun](10)'],
...
def on_click(event):
    if event.inaxes == axs[0, 0]:  
        ...
        closest_index = ((mass_increase_points['time[Myr]'] - event.xdata)**2 + 
                         (mass_increase_points['massNew[Msun](10)'] - event.ydata)**2).idxmin()
        ...
        IM: {event_info['im']}
        MassOld (Msun): {event_info['massOld[Msun]']}
        MassNew (Msun): {event_info['massNew[Msun](10)']}
        MassChange: {event_info['MassChange']}
        aOld (Rsun): {event_info['aOld[Rsun]']}
        aNew (Rsun): {event_info['aNew[Rsun]']}
        eOld: {event_info['eOld(15)']}
...
axs[1, 0].plot(cand['aOld[Rsun]'], cand['eOld(15)'])
...
filtered_df = df[df['mergMass1(25)'] > 0.001]
...
filtered_df2['MassChange'] = filtered_df2['massNew[Msun](10)'] - filtered_df2['massOld[Msun]']
...
legend.get_frame().set_edgecolor('white')  
plt.setp(legend.get_texts(), color='white')  
axs.plot(filtered_df2['time[Myr]'], filtered_df2['massNew[Msun](10)'], color='w')
...
def on_click(event):
    if event.inaxes == axs:  
        ...
        closest_index = ((mass_increase_points2['time[Myr]'] - event.xdata)**2 + 
                         (mass_increase_points2['massNew[Msun](10)'] - event.ydata)**2).idxmin()
