...
def run_system(p, v, m, R, ft=1000, dt_out=0.5, stop_dist=10000, save_trajectories=True, fname="0000000"):
    ...
    while time < ftime:
        ...
        if code.check_collision() > 0:
            # Values of code.check_collision()
            # 2 for collision/direct plunge
            # 3 for full tidal disruptions
            # Partial tidal disruptions are non-stopping, and are tracked in PTDELog
